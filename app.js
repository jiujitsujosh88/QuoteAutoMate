(() => {
  ...
  const STRINGS = {
    en: {
      ...
      'tabs.quotes': 'Quotes', 'tabs.history': 'History',
      'tabs.customers': 'Customers','tabs.presets': 'Presets',
      'tabs.analytics': 'Analytics','tabs.more': 'More',
      'more.title': 'More','more.settings': 'Settings',
      'more.business': 'Business Info','more.resources': 'Resources',
      'more.forceUpdate': 'Force Update (clear cache & reload)','more.close': 'Close',
      ...
      // Dev DB panel
      'dev.db.title':'Local Database (dev)',
      'dev.db.add':'Add Demo Quote',
      'dev.db.counts':'Show Store Counts',
      'dev.db.clear':'Clear ALL Stores'
    },
    es: {
      'app.title': 'Quote AutoMate',
      'tabs.quotes': 'Cotizaciones','tabs.history': 'Historial',
      'tabs.customers': 'Clientes','tabs.presets': 'Preajustes',
      'tabs.analytics': 'Análisis','tabs.more': 'Más',
      'more.title': 'Más','more.settings': 'Ajustes',
      'more.business': 'Información del negocio','more.resources': 'Recursos',
      'more.forceUpdate': 'Forzar actualización (borrar caché y recargar)','more.close': 'Cerrar',
      ...
      'dev.db.title':'Base de datos local (dev)',
      'dev.db.add':'Añadir cotización demo',
      'dev.db.counts':'Mostrar conteos',
      'dev.db.clear':'Borrar TODAS las tablas'
    }
  };
  ...
  function applyI18n(){
    ...
    Object.entries(ph).forEach(([pane,key])=>{
      ...
      if (!el) return;
      const exists = el.querySelector('.placeholder');
      if (!exists){
        const block = document.createElement('div');
        block.className = 'placeholder'; block.textContent = t(key);
        el.appendChild(block);
      } else {
        exists.textContent = t(key);
      }
    });
  }
  ...
  // -----------------------------
  // Dev DB panel (in Settings)
  // -----------------------------
  function mountDevDBPanel(){
    const host = document.getElementById('tab-settings');
    if (!host || host.querySelector('#dev-db-panel')) return;

    const wrap = document.createElement('div');
    wrap.id = 'dev-db-panel';
    wrap.style.marginTop = '12px';
    wrap.innerHTML = `
      <div style="margin:10px 0;font-weight:700">${t('dev.db.title')}</div>
      <div style="display:flex; gap:8px; flex-wrap:wrap">
        <button id="db-add" class="more-item" style="width:auto">${t('dev.db.add')}</button>
        <button id="db-counts" class="more-item" style="width:auto">${t('dev.db.counts')}</button>
        <button id="db-clear" class="more-item" style="width:auto">${t('dev.db.clear')}</button>
      </div>
      <div id="db-out" class="placeholder" style="margin-top:8px"></div>
    `;
    host.appendChild(wrap);

    const $out = (msg) => { const el = document.getElementById('db-out'); if (el) el.textContent = msg; };

    document.getElementById('db-add')?.addEventListener('click', async ()=>{
      const id = 'q_' + Date.now();
      await window.QAMDB.put('quotes', {
        id, createdAt: Date.now(), title: 'Demo Quote', total: Math.round(Math.random()*50000)/100
      });
      $out(`Saved demo quote: ${id}`);
    });
    document.getElementById('db-counts')?.addEventListener('click', async ()=>{
      const c = await window.QAMDB.counts();
      $out(`Counts → quotes:${c.quotes}, customers:${c.customers}, vehicles:${c.vehicles}, presets:${c.presets}, settings:${c.settings}`);
    });
    document.getElementById('db-clear')?.addEventListener('click', async ()=>{
      await Promise.all(['quotes','customers','vehicles','presets','settings'].map(s => window.QAMDB.clear(s)));
      $out('All stores cleared.');
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Init DB (from db.js), then mount dev panel
    try {
      await window.QAMDB.init();
      mountDevDBPanel();
    } catch(e){
